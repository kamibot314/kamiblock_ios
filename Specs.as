...
public class Specs {
	...
	//2017/04/11 이승훈 추가 녹음 카테고리
	public static const recordCategory:int = 13;
	//2017/06/09 이승훈 추가 모니터 카테고리
	public static const monitorCategory:int = 14;
...
//		[11, "Parameter",	parameterColor],
		...
		//2017/04/11 이승훈 수정
		//record categories 추가
		[13, "Record",		0x4a6cd4],
		//2017/06/09 이승훈 수정
		//monitor categories 추가
		//[14, "Monitor",		0x2ca5e2],
	...
	public static function IconNamed(name:String):* {
		...
		var icon:Bitmap;
		if (name == "greenFlag") { icon = Resources.createBmp('flagIcon'); icon.scaleX *= 0.25; icon.scaleY *= 0.25}
		if (name == "stop") { icon = Resources.createBmp('stopIcon'); icon.scaleX *= 0.5; icon.scaleY *= 0.5}
		if (name == "turnLeft") icon = Resources.createBmp('turnLeftIcon');
		if (name == "turnRight") icon = Resources.createBmp('turnRightIcon');
		if (icon != null) { icon.scaleX *= 0.5; icon.scaleY *= 0.5; }
		return icon;
	}

	public static var commands:Array = [
		// block specification					
		// motion								type, cat, opcode			default args (optional)
		...
		//2017/04/21 이승훈 수정 
		//pop --> recording1으로 변경
		["play sound %m.sound",					" ", 3, "playSound:",						"recording1"],
		["play sound %m.sound until done",		" ", 3, "doPlaySoundAndWait",				"recording1"],
		...
		//2017/04/20 이승훈 추가
		//Recording blocks
		["recording1",							"r", 13, "recording1", 				"recording1"],
		["recording2",							"r", 13, "recording2", 				"recording2"],
		["recording3",							"r", 13, "recording3", 				"recording3"],
		["recording4",							"r", 13, "recording4", 				"recording4"],
		["recording5",							"r", 13, "recording5", 				"recording5"],
