...
package scratch {
	...
	import soundedit.PalleteSoundEditor;
	import soundedit.PalleteWaveformView;
...
//	public static var paletteSoundsPartInitChk:Boolean = true;
	...
	public function PaletteBuilder(app:Main) {
		this.app = app;
	}
	
//	public function getPaletteSoundsPartInitChk():Boolean{
//		return paletteSoundsPartInitChk;
//	}
//	
//	public function setPaletteSoundsPartInitChk(value:Boolean):void{
//		paletteSoundsPartInitChk=value;
//	}
	...
	public function showBlocksForCategory(selectedCategory:int, scrollToOrigin:Boolean, shiftKey:Boolean = false):void {
		...
		nextY = 7;
		app.isMonitorScreenOn = false;
		if (selectedCategory == Specs.dataCategory) return showDataCategory();
		//2017/06/09 이승훈 수정
		//모니터 카테고리 추가
		if (selectedCategory == Specs.monitorCategory){
			app.isMonitorScreenOn = true;
			return showMonitorCategory();
		}
...
//			addExtensionSeparator(ext);
			if(ext.showBlocks) addBlocksForExtension(ext);
		}
	...
	private function showDataCategory():void {
		...
		if (varNames.length > 0) {
			...
			for each (var n:String in varNames) {
				...
				addVariableInScriptPane(idx,n,false);
...
//		addLine(x, nextY + 9, 180 - x);

//		var indicator:IndicatorLight = new IndicatorLight(ext);
//		indicator.addEventListener(MouseEvent.CLICK, function(e:Event):void {Main.app.showTip('extensions');}, false, 0, true);
...
//	}
	...
	private function addExtensionSeparator(name:String):void {
			nextY += 7;
			var titleButton:Sprite = UIPart.makeButtonLabel(Translator.map(name), CSS.textColor, false);
			titleButton.x = 5;
			titleButton.y = nextY;
			app.palette.addChild(titleButton);
			var x:int = titleButton.width + 12;
			addLine(x, nextY + 9, 220 - x);
			nextY += titleButton.height + 10;
	}
	...
	private function addBlocksForExtension(ext:ScratchExtension):void {
		...
		for each (var spec:Array in ext.blockSpecs) {
			if (spec.length >= 3) {
				...
				switch (spec[2]) {
					case "runBlockMove":
					case "runBlockTurnLeft":
					case "runBlockTurnRight":
					case "runBlockTurnBack":
						blockColor = 0x672D79;
						break;
					default:
						blockColor = Specs.extensionsColor;
						break;
				}
				var block:Block = new Block(spec[1], spec[0], blockColor, op, defaultArgs);
				var showCheckbox:Boolean = (spec[0] == 'r' && defaultArgs.length == 0);
				if (showCheckbox) addReporterCheckbox(block);
				if (spec[2] == "runBlockMove") addExtensionSeparator(Translator.map("KamiBot MapBoard"));
				addItem(block, showCheckbox);
				if (spec[2] == "runBlockTurnBack") addExtensionSeparator(Translator.map("KamiBot"));
			} else {
	...
	//2017/03/28 이승훈
	private function addVariableInScriptPane(idx:int,varName:String, isList:Boolean):void {
//		if(app.variablePart.numChildren>0){
//			if(idx==0){
//				for(var i:int;i<app.variablePart.numChildren;i++){
//					app.variablePart.removeChildren(i);
//				}
//			}
//		}
//		app.variablePart.createVariable(idx,varName,isList);
	}
	
	//2017/04/11 이승훈
	//레코드 팔레트 추가
	//팔레트 선택된 녹음파일 Index 추가
	public function showRecordPalette(scrollToOrigin:Boolean, shiftKey:Boolean = false):void 
	{
		trace("showRecordPalette In!!!!");
		//팔레트 초기화
		if (app.palette == null) return;
		app.palette.clear(scrollToOrigin);
		nextY = 80;
		//paletteSoundsPart = new PaletteSoundsPart(app);
		//2017/06/08 이승훈 수정
		addLine(7,72,app.palette.width-25);
		var catColor:int = Specs.blockColor(13);
		paletteSoundsPart = app.soundsPart;
		paletteSoundsPart.editor = new PaletteSoundEditor(app, paletteSoundsPart);
		addBlocksForRecordCategory(13, catColor);
		paletteSoundsPart.refresh();
	}
	
	private function addBlocksForRecordCategory(category:int, catColor:int):void {
		var cmdCount:int;
		var targetObj:ScratchObj = app.viewedObj();
		for each (var spec:Array in Specs.commands) {
			if ((spec.length > 3) && (spec[2] == category))
	...
	//2017/06/09 이승훈 추가
	private function showMonitorCategory():void {
		app.monitorPart.nextY=0;
		app.monitorPart.addMonitorText('battery level',0);
		app.monitorPart.addMonitorText('ultrasonic sensor',0);
		app.monitorPart.addMonitorText('infrared sensor No.1',0);
		app.monitorPart.addMonitorText('infrared sensor No.2',0);
		app.monitorPart.addMonitorText('infrared sensor No.3',0);
		app.monitorPart.addMonitorText('infrared sensor No.4',0);
		app.monitorPart.addMonitorText('infrared sensor No.5',0);
	}
