...
package scratch {
	...
	//----------
		public function stopAll():void {
			...
			ConnectionManager.sharedManager().clearTimeoutTimer();
...
//						app.libraryPart.showVideoButton();
		...
		public function installEmptyProject():void {
			...
			//21d7c8705a0fdeea32affb616ee6c984 //cat1
			//766f6e15dd6a8a3802e2903686594e6b //BitBlock
			//mcat.json
			io.fetchSprite("main.json", whenDone);
		}
		...
		public function selectProjectFile():void {
			...
			d.addTitle(Translator.map('Select File') + '...');
			d.addButton('Cancel', function():void {
				d.cancel();
			});
			...
			try
			{
				...
				if (array.length == 0) {
					DialogBox.notify("File not found","Cannot found sb2 files at 'KamiBlock' folder.\nplease put your project file(s) at 'KamiBlock' folder");
				} else {
					for each (var f:File in array) 
					{
						if (f.extension == "sb2" || f.extension == "sb") {
							d.addButtonExt(f.name, f.name, function(param:String ):void {
								var file:File = dir.resolvePath(param);
								selectedProjectFile(file.nativePath);
							});
						}
					}
			...
			{
				DialogBox.notify("File not found","Cannot found sb2 files at 'KamiBlock' folder.\nplease put your project file(s) at 'kamibot_scratch' folder");
			}
		}
...
//				if (v.) v.(newName);
		...
		public function checkGreenFlag():Block {
			...
			for (var i:int = stage.numChildren - 1; i >= 0; i--) {
				...
				if (o is ScratchObj) {
					for each (stack in ScratchObj(o).scripts) {
						if (stack.op == "whenGreenFlag") return stack;
					}
				}
			}
