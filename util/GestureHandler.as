...
public class GestureHandler {
	...
	private const LONG_CLICK_MSECS:int = 2000;
...
//	private var mouseDownEventX:Number;
//	private var mouseDownEventY:Number;
	...
	public function GestureHandler(app:Main, inIE:Boolean) {
		this.app = app;
	...
	public function setDragClient(newClient:DragClient, evt:MouseEvent):void {
		...
		ComboBox.removeMenusFrom(stage);
...
//		}
	...
	public function rightMouseDown(x:int, y:int, isChrome:Boolean):void {
		...
		//2017/04/18 이승훈 추가
		ComboBox.removeMenusFrom(stage);
		var comboTarget:* = findTargetFor('combo', app, x, y);
		if (!comboTarget) return;
		try { var combo:ComboBox = comboTarget.menu(new MouseEvent('right click')) } catch (e:Error) {}
		if (combo) combo.showOnStage(stage, x, y);
		if (!isChrome) ComboBox.removeMenusFrom(stage); // hack: clear menuJustCreated because there's no rightMouseUp
...
//		mouseDownTime = getTimer();
		mouseDownEvent = evt;
//		mouseDownEventX = evt.stageX;
//		mouseDownEventY = evt.stageY;
		gesture = "unknown";
		mouseTarget = null;
		...
		var t:* = evt.target;
		if ((t is TextField) ) { 
			var v:TextField = (t as TextField);
			var v2:BlockArg = (v.parent as BlockArg);
			if (v2 != null) {
				v2.startEditing();
			}
			return;
		...
		if ((t is BlockArg)) {
			var v3:BlockArg = (t as BlockArg);
			v3.startEditing();
		}
		
		if ((t is TextField) && (TextField(t).type == TextFieldType.INPUT)) {trace("TextFieldType.INPUT");;return;};
...
//		trace("mouseDownEvent.stageX = " + mouseDownEvent.stageX);
//		trace("mouseDownEventX = " + mouseDownEventX);
//		trace("evt.stageX = " + evt.stageX);
//		
//		trace("(Math.abs(mouseDownEventX - evt.stageX) = " + (Math.abs(mouseDownEventX - evt.stageX)));
//		trace("(Math.abs(mouseDownEventY - evt.stageY) = " + (Math.abs(mouseDownEventY - evt.stageY)));
		
//		if (((getTimer() - mouseDownTime) < LONG_CLICK_MSECS) &&
//			mouseDownEvent != null &&
//			(Math.abs(mouseDownEventX - evt.stageX) < 10) &&
//			(Math.abs(mouseDownEventY - evt.stageY) < 10)) {
//			trace("rightMouseClick");
//			rightMouseClick(evt);
//		}
	...
	private function grab(obj:*, evt:MouseEvent):void {
		...
		if (obj is Block) {
			...
			trace("originalParent = " + originalParent);
			if (originalParent != null && (originalParent is ScriptsPane || originalParent is Block)) {//parent is ScriptsPane - not attached stack
				b.scaleX = util.ApplicationManager.sharedManager().scale * app.scriptsPane.scaleX;
				b.scaleY = util.ApplicationManager.sharedManager().scale * app.scriptsPane.scaleX;
			} else {
				b.scaleX = util.ApplicationManager.sharedManager().scale;
				b.scaleY = util.ApplicationManager.sharedManager().scale;
			}
