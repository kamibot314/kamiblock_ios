...
public class BlockIO {
	...
	private static function arrayToBlock(cmd:Array, undefinedBlockType:String, forStage:Boolean = false):Block {
		...
		if (cmd[0] == Specs.CALL) {
			...
			if (cmd[0].indexOf("runArduino") !== -1) cmd[0] = "whenGreenFlag";
			var spec:Array = specForCmd(cmd, undefinedBlockType);
			var label:String = spec[0];
			if(forStage && spec[3] == 'whenClicked') label = 'when Stage clicked';
			//runArduino to whenGreenFlag
			var blockColor:int;
			
			if ((spec[3] as String).indexOf("runBlockMove") !== -1 || 
				(spec[3] as String).indexOf("runBlockTurnLeft") !== -1 || 
				(spec[3] as String).indexOf("runBlockTurnRight") !== -1 || 
				(spec[3] as String).indexOf("runBlockTurnBack") !== -1) {
				blockColor = 0x672D79;
			} else {
				blockColor = Specs.blockColor(spec[2]);
			}
			
			b = new Block(label, spec[1], blockColor, spec[3]);
		}
	...
	private static function specialCmd(cmd:Array, forStage:Boolean):Block {
		...
		case 'KeyEventHatMorph':
			...
			b.setArg(0, cmd[1]);
			return b;
		case 'KeyEventHatMorphReleased':
			b = new Block('when %m.key key released', 'h', controlColor, 'whenKeyReleased');
			b.setArg(0, cmd[1]);
			return b;
		case 'stopScripts':
