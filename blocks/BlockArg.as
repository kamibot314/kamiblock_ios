...
public class BlockArg extends Sprite {
	...
	public static var vOffset:uint = 0;
	...
	public function BlockArg(type:String, color:int, editable:Boolean = false, menuName:String = '') {
		...
		if (editable || isNumber || (type == 'm')) { // add a string field
			...
			if (isNumber) {
				field.restrict = '0-9e.\\-'; // restrict to numeric characters
				field.softKeyboard = SoftKeyboardType.NUMBER;
			}
			if (editable) {
...
//			field.setSelection(0, field.text.length);
			...
			field.addEventListener(KeyboardEvent.KEY_DOWN, 
	...
	private function textChanged(evt:*):void {
		...
		var w:int = Math.max(getMinWidth(), field.textWidth + 6 + padding);
		if (menuIcon) menuIcon.x = w - menuIcon.width - 3;
		base.setWidth(w);
		base.redraw();
		if (parent is Block) Block(parent).fixExpressionLayout();
		
		if (evt && Main.app) Main.app.setSaveNeeded();
	}
	
	private function getMinWidth():uint {
		var tf:TextField = new TextField();
		tf.autoSize = TextFieldAutoSize.LEFT;
		Block.argTextFormat.bold = true;
		tf.defaultTextFormat = Block.argTextFormat;
		tf.text = "...";//defalut text length (variable length font)
		return tf.width;
	}

	private function invokeMenu(evt:MouseEvent):void {
		if ((type != "m") && (menuIcon != null) && (evt.localX <= menuIcon.x)) return;
