...
public class Translator {
	...
	public static function initializeLanguageList():void {
		...
		function saveLanguageList(data:String):void {
			...
			var lang:String = SharedObjectManager.sharedManager().getObject("lang",null);
			if (lang == null) {
				lang = "en";
				for each (var lang2:Object in languages) 
				{ 
					if(lang2[0] == Capabilities.language) lang = Capabilities.language;
				}
			}
		...
		Main.app.server.getLanguageList(saveLanguageList);
	}
	...
	public static function setLanguage(lang:String):void {
		...
		else if ('set block size' == lang) { fontSizeMenu(); return; }
		else{
		...
		isEnglish = true;
		setFontsFor(lang);
		if ('en' == lang) Main.app.translationChanged(); // there is no .po file English
		else Main.app.server.getPOFile(lang, gotPOFile);

		Main.app.server.setSelectedLang(lang);
	}
	...
	private static function fontSizeMenu():void {
		...
		for (var i:int = 8; i <= 50; i+=2) m.addItem(i.toString(), i);
...
//if (!isEnglish && (s.indexOf('%') == -1)) return s.toUpperCase(); // xxx for testing only; comment out before pushing!
	...
	private static function checkBlockSpec(spec:String):void {
		...
		if (!argsMatch(extractArgs(spec), extractArgs(translatedSpec))) {
			Main.app.log('Block argument mismatch:');
			Main.app.log('    ' + spec);
			Main.app.log('    ' + translatedSpec);
