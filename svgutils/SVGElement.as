...
public class SVGElement {
	...
	public function set transform(v:Matrix):void{
		_transform = v;
	}
...
//		if (transform) bmp.transform.matrix = transform;
	...
	public function renderSVGTextOn(tf:SVGTextField):void {
...
//				fmt.font = 'Helvetica';
		...
		tf.text = text;
		tf.textfield.width = tf.textWidth + 6;
		tf.textfield.height = tf.textHeight + 4;
		...
		tf.alpha = alpha();
		if (transform) {
			tf.original.transform.matrix = transform;
//			tf.textfield.rotation = tf.rotation;
		}
	}
	public function renderTextOn(tf:TextField):void {
		// Render a text element on the given TextField.
		// For now, always use an embedded font to allow rotation.
		const useEmbeddedFont:Boolean = false;
		if (!text) return;
		var fmt:TextFormat = new TextFormat(
			getAttribute('font-family', 'Helvetica'),
			getAttribute('font-size', 24),
		...
		tf.alpha = alpha();
		if (transform) {
			tf.transform.matrix = transform;
			tf.rotationZ = tf.rotation;
		}
	}
