...
public class ExtensionManager {
	...
	public function specForCmd(op:String):Array {
		...
		for each (var ext:ScratchExtension in extensionDict) {
			...
			op = prefix + op.substring(op.indexOf(".") + 1);
...
//	}
	...
	public function unloadAllExtension():void{
		for each(var ext:Object in _extensionList){
			unloadRawExtension(ext);
		}
	}
	...
	public function checkExtensionSelected(name:String):Boolean{
		return (SharedObjectManager.sharedManager().getObject("extension","KamiBot") == name);
	}
	
	public function setExtensionSelected(name:String):Boolean{
		return SharedObjectManager.sharedManager().setObject("extension",name);
	}
...
//		_extensionList = [];
//		if(File.applicationStorageDirectory.resolvePath("mBlock/ext/").exists){
//			var docs:Array =  File.applicationStorageDirectory.resolvePath("mBlock/ext/").getDirectoryListing();
...
//		var srcFile:File = File.applicationDirectory.resolvePath("ext/");
//		for each(var sf:File in srcFile.getDirectoryListing()){
//			var tf:File = File.applicationStorageDirectory.resolvePath("ext/"+sf.name);
//			if(!tf.exists){
//				sf.copyTo(tf,true);
//			}else{
...
//	}
	public function importExtension():void {
		_extensionList = [];
		if(File.applicationDirectory.resolvePath("ext/").exists){
			var docs:Array =  File.applicationDirectory.resolvePath("ext/").getDirectoryListing();
			for each(var doc:File in docs){
				if(!doc.isDirectory){
					continue;
				}
...
//		var tf:File = File.applicationStorageDirectory.resolvePath("mBlock/firmware/mblock_firmware/");
//		srcFile.copyTo(tf,true);
//		var botFile:File = File.applicationDirectory.resolvePath("firmware/mbot_firmware/");
//		var bottf:File = File.applicationStorageDirectory.resolvePath("mBlock/firmware/mbot_firmware/");
//		botFile.copyTo(bottf,true);
//		var hexFile:File = File.applicationDirectory.resolvePath("tools/hex/");
//		var htf:File = File.applicationStorageDirectory.resolvePath("mBlock/tools/hex/");
//		hexFile.copyTo(htf,true);
//		var localsFile:File = File.applicationDirectory.resolvePath("locale/");
//		var ltf:File = File.applicationStorageDirectory.resolvePath("mBlock/locale/");
...
//	}
	...
	public function reporterCompleted(extensionName:String, id:Number, retval:*):void {
		trace("reporterCompleted retval = " + retval);
		var ext:ScratchExtension = extensionDict[extensionName];
		if (ext == null) return; // unknown extension
	...
	public function loadExtension(extName:String):Boolean
	{
		unloadAllExtension();
		var ext:Object = findExtensionByName(extName);
		if (ext != null) {
			setExtensionSelected(extName);
			loadRawExtension(ext);
			return true;
		}
		return false;
	...
	private function loadRawExtension(extObj:Object):void {
		checkExtensionSelected(extObj.extensionName);
...
//		parseTranslators(ext);
		Main.app.extensionManager.parseAllTranslators();
	...
	public function loadSavedExtensions(savedExtensions:Array):void {
		...
		for each (var extObj:Object in savedExtensions) {
			...
			loadExtension(extObj.extensionName);
		}
//		return;
...
//				}
//			}
			trace("extName : "+extName);
			trace("primOrVarName : "+primOrVarName);
			trace("args : "+args);
	...
	public function call(extensionName:String, op:String, args:Array):void {
		...
		if (ext == null||(ext.useSerial&&!SerialManager.sharedManager().connected)){
			return;
		}
	...
	public function request(extensionName:String, op:String, args:Array, b:Block):void {
		var ext:ScratchExtension = extensionDict[extensionName];
		if (ext == null||(ext.useSerial&&!SerialManager.sharedManager().connected)||app.runtime.isRequest){
...
//		}
	...
	private function httpRequest(ext:ScratchExtension, op:String, args:Array, b:Block):void {
		...
		if(ext.useSerial){
			...
			var objs:Array = Main.app.extensionManager.specForCmd(ext.name+"."+op);
