...
//			languageButton.scaleX = 0.25;
//			languageButton.scaleY = 0.25;
//			addTextButtons();
//			addToolButtons();
		}
...
//				removeChild(deviceMenu);
		...
		public function updateVersion():void{
			if(ParseManager.sharedManager().firmVersion === ""){
				updateState("Need to Firmware upload");
			}else{
				var hardwareVer:uint = ParseManager.sharedManager().firmVersion.split(".")[1];
				updateState(Translator.map("Firmware Connected") + " v" +  ParseManager.sharedManager().firmVersion);
			}
		}
...
//			addChild(deviceMenu = makeMenuButton(Translator.map('Boards')+" ( "+BoardManager0.getInstance().board+" )",app.showBoardMenu,true));
...
//			}
		...
		public function setConnectedTitle(title:String = null):void{
			removeChild(connectMenu);
			if (title == null) addChild(connectMenu = makeMenuButton(Translator.map("Connected"), app.showConnectMenu, true));
			else  addChild(connectMenu = makeMenuButton(title, app.showConnectMenu, true));
