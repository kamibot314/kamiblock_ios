...
public class MediaLibrary extends Sprite {
	...
	public function MediaLibrary(app:Main, type:String, whenDone:Function) {
		this.app = app;
		this.assetType = type;
...
spriteFeaturesFilter.visible = false; // disable features filter for now
	...
	private function convertAndUploadImageOrSprite(fName:String, data:ByteArray):void {
		...
		function decodeError():void {
			DialogBox.notify('Error decoding image', 'Sorry, Scratch was unable to load the image '+fName+'.', Main.app.stage);
		}
		var costumeOrSprite:*;
		var fExt:String = '';
	...
	public function convertAndUploadSound(sndName:String, data:ByteArray):void {
		function uploadComplete():void {
			app.removeLoadProgressBox();
			whenDone(snd);
		}
