...
public class MediaPane extends ScrollFrameContents {
	...
	public function MediaPane(app:Main, type:String):void {
		trace("MediaPane In!!!");
		trace("type : "+type);
		this.app = app;
		isSound = (type == 'sounds');
	...
	public function updateSelection():Boolean {
		if (isSound) {
			trace("app.viewedObj().sounds : "+app.viewedObj().sounds);
	...
	private function costumeItems():Array {
		...
		for each (var c:ScratchCostume in viewedObj.costumes) {
			result.push(Main.app.createMediaInfo(c, viewedObj));
		}
		return result;
	}
	...
	private function soundItems():Array {
		...
		var result:Array = [];
		var viewedObj:ScratchObj = app.viewedObj();
		trace("viewedObj : "+viewedObj);
		for each (var snd:ScratchSound in viewedObj.sounds) {
			trace("soundItems() for In!!!!!!");
			result.push(Main.app.createMediaInfo(snd, viewedObj));
		}
		return result;
	}
	...
	private function updateSoundSelection():void {
		var viewedObj:ScratchObj = app.viewedObj();
		trace("updateSoundSelection() In~~~~~");
		trace("app.viewedObj() : "+app.viewedObj());
		trace("viewedObj.sounds.length : "+viewedObj.sounds.length);
		if ((viewedObj == null) || !isSound) return;
		if (viewedObj.sounds.length < 1) return;
		//if (!this.parent || !this.parent.parent) return;
		var sp:PaletteSoundsPart = app.soundsPart;
		if (sp == null) return;
		sp.refresh();
		//2017/04/20 이승훈 수정
		//사운드 선택 주석처리
//	 	sp.setCurrentIdx(Math.min(sp.getCurrentIdx(), viewedObj.sounds.length - 1));
//		var current:ScratchSound = viewedObj.sounds[sp.getCurrentIdx()] as ScratchSound;
