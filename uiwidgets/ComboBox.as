...
public class ComboBox extends Sprite {
	...
	public function ComboBox(clientFunction:Function = null, menuName:String = '', color:int = 0xA0A0A0, itemHeight:int = 28) {
		this.clientFunction = clientFunction;
		this.menuName = menuName;
		this.color = color;
		this.itemHeight = itemHeight;
		this.mouseChildren = true;
		this.scaleX =  util.ApplicationManager.sharedManager().scale;
		this.scaleY =  util.ApplicationManager.sharedManager().scale;
	}

	public function addItem(label:String, value:* = null, enabled:Boolean = true, checkmark:Boolean = false):void {
		var last:ComboBoxItem = allItems.length ? allItems[allItems.length-1] : null;
		var newItem:ComboBoxItem = new ComboBoxItem(this, label, value, enabled);
	...
	public function showOnStage(stage:Stage, x:int = -1, y:int = -1):void {
		if (stringCollectionMode) {
			for each (var item:ComboBoxItem in allItems) {
	...
	private function scrollBy(delta:int):void {
		...
		for (var i:int = firstItemIndex; i < allItems.length; i++) {
			var item:ComboBoxItem = allItems[i];
			addChild(item);
			item.x = 1;
			item.y = nextY;
			nextY += item.height;
