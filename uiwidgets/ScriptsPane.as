...
public class ScriptsPane extends ScrollFrameContents {
	...
	public function ScriptsPane(app:Main) {
		this.app = app;
...
//			app.scriptsPart.parseArduinoCodeByStage();
	}
...
return true; // xxx disable this check for now; it was causing confusion at Scratch@MIT conference
	...
	private function nearestTargetForBlockIn(b:Block, targets:Array):Array {
		...
		threshold *= util.ApplicationManager.sharedManager().scale;
	...
	public function handleDrop(obj:*):Boolean {
		...
		if (b) {
			if (b.op == "whenGreenFlag") {
				var isFlag:Block = app.runtime.checkGreenFlag();
				trace("isFlag = " + isFlag);
				if (isFlag != null && isFlag != b) return false;
			}
		}
